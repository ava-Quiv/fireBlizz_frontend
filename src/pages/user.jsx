...
const CustomersDashboard = () => {
  ...
  useEffect(() => {
    let url = 'http://localhost:5000/add/customer/seeCustomer'; // Default URL for 'all' customers

    if (filter === 'active') {
      url = 'http://localhost:5000/add/customer/active';
    } else if (filter === 'rejected') {
      url = 'http://localhost:5000/add/customer/rejected';
    } else if (filter === 'suspended') {
      url = 'http://localhost:5000/add/customer/suspended';
    } else if (filter === 'pending') {
      url = 'http://localhost:5000/add/customer/pending';
    }
  ...
  const handleApprove = async (customerId) => {
    try {
      await axios.put(`http://localhost:5000/add/customer/approve/${customerId}`);
  ...
  const handleReject = async (customerId) => {
    try {
      await axios.put(`http://localhost:5000/add/customer/reject/${customerId}`);
      setCustomers(customers.map(customer =>
  ...
  const handleSuspend = async (customerId) => {
    try {
      await axios.put(`http://localhost:5000/add/customer/suspend/${customerId}`);
      setCustomers(customers.map(customer =>
  ...
  const handleReactivate = async (customerId) => {
    try {
      await axios.put(`http://localhost:5000/add/customer/reactivate/${customerId}`);
      setCustomers(customers.map(customer =>
        customer._id === customerId ? { ...customer, status: 'active' } : customer
      ));
    } catch (error) {
